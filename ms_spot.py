    ...
    def __init__(self, encoder, args, second_encoder=None):
        ...
        with torch.no_grad():
            ...
            # and used to define the permutation sequence. Here we only need the LAST layers H*W shape
                sample_input = torch.rand(args.batch_size, args.img_channels, args.image_size, args.image_size).to('cuda')
    ...
    def forward_encoder(self, x, encoder):
        ...
        if self.which_encoder == "hf_diffusion":
           ...
           # return [tensor.view(tensor.size(0), 384, 64, 64) for tensor in features_list]
            return  [tensor.transpose(1, 2) for tensor in features_list]
