...
class MSSPOT(nn.Module):
    def __init__(self, encoder, args, second_encoder=None):
        ...
        with torch.no_grad():
            ...
            if args.which_encoder == "hf_diffusion":
            # parse sample input to get a list of the shape of the encoder ouput at different layers
            # we need this for the d_model parameter (initialize slot attention to expect the right input shape, extract a list of d_model from
            # the encoder) and num_tokens, which is usually the number of patches generated by the encoder 
            # and used to define the permutation sequence. Here we only need the LAST layers H*W shape
                sample_input = torch.rand(2, args.img_channels, args.image_size, args.image_size).to('cuda')
                sample_output = self.forward_encoder(sample_input, self.encoder) # list of [2, 384, 4096]
                
                last_layer = sample_output[-1]
                d_model = last_layer.shape[2]
                num_tokens = last_layer.shape[1] # H*W     
            else:
    ...
    def forward_encoder(self, x, encoder):
        ...
        elif self.which_encoder == "hf_diffusion":
            x = x
        else:
        ...
        # last_layer = None
        if self.which_encoder == "hf_diffusion":
            batch_size = 2
            
            all_features = []
            for i in range(0, x.shape[0], batch_size):
                sub_batch = x[i:i+batch_size]
                sub_batch_features = self.encoder(sub_batch)
                all_features.append(sub_batch_features)
            features_per_type = list(zip(*all_features))
            final_features = [torch.cat(f_list, dim=0) for f_list in features_per_type]
