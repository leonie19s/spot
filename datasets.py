...
class COCO2017CACHE(Dataset):
 ...
 89, 90]
    ...
    def __init__(self, root, split='train', year='2017', image_size=224, mask_size=224, return_gt_in_train=False, layers =[8,9,10,11]):
        super().__init__()
        ...
        self.root_train = "/fastdata/vilab01/coco_cache"
        self.root_val = "/fastdata/vilab01/coco_cache_val"
    ...
    def load_layers_for_img(self, img_cache_path):
        if self.split == "train":
            path = os.path.join(self.root_train, img_cache_path)
        elif self.split == "val":
            path = os.path.join(self.root_val, img_cache_path)
        else:
            raise
        layer_tensors = []
        ...
        # Sort by layer number for consistency
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            ...
            encoded_layers = self.load_layers_for_img(base_filename)
        ...
        if self.split == "train" and (self.return_gt_in_train is False):
            ...
            return img, mask_instance, mask_class, mask_ignore, encoded_layers       
        elif self.split =='val':
