...
class MSA_SPOT(nn.Module):
    def __init__(self, encoder, args, second_encoder=None):
        ...
        self.slot_attention_scales = args.slot_attention_scales
        ...
        with torch.no_grad():
            ...
            x = self.forward_encoder(x, self.encoder) # can remain -1 since we only interested in final size for tokens and d_model
    ...
    def forward_encoder(self, x, encoder):
        ...
        # For storing outputs of individual encoder blocks for later multi-scale slot attention computation
        outputs = []
        ...
        for blk_idx, blk in enumerate(encoder.blocks):
            x = blk(x)

            # Append intermediate results and last block as well
            if blk_idx >= len(encoder.blocks) - self.slot_attention_scales:
                outputs.append(x)

        if self.encoder_final_norm: # The DINOSAUR paper does not use the final norm layer according to the supplementary material.
            outputs[-1] = encoder.norm(outputs[-1])
        ...
        # remove the [CLS] and (if they exist) registers tokens 
        for i in range(len(outputs)):
            outputs[i] = outputs[i][:, offset :]
    ...
    def forward(self, image):
        ...
        with torch.no_grad():
            if self.second_encoder is not None:
                emb_target = self.forward_encoder(image, self.second_encoder)[-1]
            else:
                emb_target = emb_input[-1].clone().detach()
        # emb_target shape: B, N, D
        # emb_target is the output of the last layer of the encoder, hence [-1] of outputs of blocks
