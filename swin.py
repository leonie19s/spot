...
class SwinTransformerV2(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            x = layer(x)
            features.append(x) # collect feature layers after each stage
    ...
    def forward(self, x):
        ...
        x = self.head(x) # i think we can ignore the head
        return x
...
class SwinTransformerV2ForSimMIM(SwinTransformerV2):
    ...
    def forward(self, x): # modified version without the masking logic
        x = self.patch_embed(x)
        ...
        for layer in self.layers:
            x = layer(x)
            features.append(x) # collect the layer features
        x = self.norm(x)
        features[-1] = x
        x = x.transpose(1, 2) # TODO: should we reshape them?
        B, C, L = x.shape
        H = W = int(L ** 0.5)
        x = x.reshape(B, C, H, W)
...
def build_swin_model(img_size):
    encoder = SwinTransformerV2ForSimMIM(
        img_size = img_size,
        #img_size=img_size,# TODO: in SIMMIM config this is set to 192.
        patch_size=4,# TODO: to what should this be set? 47?
        in_chans=3,
